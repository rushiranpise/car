...
class Ultrasonic():
    ...
    def __init__(self, trig, echo, timeout=0.02):
        if not isinstance(trig, Pin):
            raise TypeError("trig must be sensorhat.Pin object")
        if not isinstance(echo, Pin):
            raise TypeError("echo must be sensorhat.Pin object")
...
class RGB_LED():
    ...
    def __init__(self, r_pin: PWM, g_pin: PWM, b_pin: PWM, common: int = 1):
        ...
        :type r_pin: sensorhat.PWM
        :param g_pin: PWM object for green
        :type g_pin: sensorhat.PWM
        :param b_pin: PWM object for blue
        :type b_pin: sensorhat.PWM
        ...
        if not isinstance(r_pin, PWM):
            raise TypeError("r_pin must be sensorhat.PWM object")
        if not isinstance(g_pin, PWM):
            raise TypeError("g_pin must be sensorhat.PWM object")
        if not isinstance(b_pin, PWM):
            raise TypeError("b_pin must be sensorhat.PWM object")
...
class Buzzer():
    ...
    def __init__(self, buzzer: Union[PWM, Pin]):
        ...
        :type pwm: sensorhat.PWM/sensorhat.Pin
        """
        if not isinstance(buzzer, (PWM, Pin)):
            raise TypeError(
                "buzzer must be sensorhat.PWM or sensorhat.Pin object")
        self.buzzer = buzzer
        self.buzzer.off()
    def on(self):
        """Turn on buzzer"""
    ...
    def __init__(self, pin0: ADC, pin1: ADC, pin2: ADC, reference: int = None):
        ...
        for i, pin in enumerate(self.pins):
            if not isinstance(pin, ADC):
                raise TypeError(f"pin{i} must be sensorhat.ADC")

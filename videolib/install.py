...
import time
import threading
sys.path.append('./videolib')
import pwd
owner_uid = os.stat(__file__).st_uid
user_name = pwd.getpwuid(owner_uid).pw_name
from version import __version__
print("Start installing videolib %s for user %s"%(__version__ ,user_name))
...
def install():
    ...
    print("Install videolib python package")
    do(msg="pip3 install ./",
        cmd=f'pip3 install ./ {_is_bsps}')
    do(msg="cleanup",
        cmd='rm -rf videolib.egg-info')
    ...
    do(msg="create dir",
        cmd='mkdir -p /opt/videolib'
        + ' && chmod 774 /opt/videolib'
        + f' && chown -R {user_name}:{user_name} /opt/videolib'
        )
